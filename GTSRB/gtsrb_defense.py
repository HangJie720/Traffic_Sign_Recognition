...
from squeeze import binary_filter_tf, median_filter_np
...
def gtsrb_whitebox(attack, mu, sigma, batch_size, eps, steps, kappa, alpha):
    ...
    logits = model(x)
    logits_bin = model(binary_filter_tf(x))
    ...
    calculate_accuracy_adv_feature_squeezing(sess, x, one_hot_y, logits, logits_bin, X_adv, y_test, eval_params=eval_params)
    display_leg_adv_sample(X_test, X_adv)
    # pair_visual(X_test, X_adv)

def calculate_accuracy_adv_feature_squeezing(sess, x, y, logits, logits_bin, X_adv, y_test, eval_params=None):
    ...
    accuracy, pred = model_test_eval(sess, x, y, logits_bin, X_adv, y_test, args=eval_params)
    print('Test accuracy of white-box on feature squeezing adversarial '
          'examples: {:.3f}'.format(accuracy))

def calculate_accuracy_adv_spatial_smoothing(sess, x, y, predictions, y_test, X_test, X_test_adv, eval_param=None):
    ...
    for width in range(1, 11):
        ...
        for height in range(1, 11):
            ...
            accuracy_leg, _ = model_test_eval(sess, x, y, predictions, X_squeezed, y_test, args=eval_param)
            accuracy_mal, _ = model_test_eval(sess, x, y, predictions, X_adv_squeezed, y_test, args=eval_param)
